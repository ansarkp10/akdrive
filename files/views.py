...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # Only this single hardcoded user is allowed to log in
        if username == 'ansar' and password == 'ansar123':
            from django.contrib.auth.models import User
            user, created = User.objects.get_or_create(username='ansar')
            if created:
                user.set_password('ansar123')
                user.save()
            user.backend = 'django.contrib.auth.backends.ModelBackend'
            login(request, user)
            return redirect('dashboard')  # Ensure this URL name exists
